	...
	if($ma1 == $ma2){								
		if($ca1 == $ca2){						
			...
			unless($af_table{$locus}{$pog1}){
				$af_table{$locus}{$pog1} = $af_table{$locus}{min};
			}	
			if($fa1 == $fa2){				
				$pi = 1/$af_table{$locus}->{$pog1};
			}else{				
				$pi = 0.5/$af_table{$locus}->{$pog1};
			}
		}
		if($ca1 != $ca2){						
			if($ca1 == $ma1){					
	...
	if($ma1 != $ma2){									
		if($ca1 == $ca2){							
			...
			if(($ca1 == $ma1) && ($ca2 == $ma2)){					
				...
				unless($af_table{$locus}{$pog2}){
					$af_table{$locus}{$pog2} = $af_table{$locus}{min};
				}
				if($fa1 == $fa2){				
					$pi = 1/($af_table{$locus}-> {$pog1} + $af_table{$locus}-> {$pog2});
				}else{				
					if(($fa1 == $pog1) && ($fa2 == $pog2)){	
						$pi = 1/($af_table{$locus}-> {$pog1} + $af_table{$locus}-> {$pog2});
					}else{	
						$pi = 0.5/($af_table{$locus}-> {$pog1} + $af_table{$locus}-> {$pog2});
					}
				}
			}else{
				if((($ca1 == $ma1) && ($ca2 != $ma2)) || ($ca1 == $ma2)){	
					$pog1 = $ca2;
				}
				if(($ca2 == $ma1) || (($ca2 == $ma2) && ($ca1 != $ma1))){	
					$pog1 = $ca1;
				}
				unless($af_table{$locus}{$pog1}){
					$af_table{$locus}{$pog1} = $af_table{$locus}{min};
...
sub get_pi_tri_father_mutation{
	...
	if($ma1 == $ma2){								
		if($ca1 == $ca2){
			...
			if($fa1 == $fa2){
				$fs1 = abs($fa1 - $pog1);											
				$pi = (1*$father_mutation*0.5*0.1**($fs-1))/$af_table{$locus}->{$pog1};
			}
			if($fa1 != $fa2){	
				$fs1 = abs($fa1 - $pog1);
				$fs2 = abs($fa2 - $pog1);
				if($fs1 < $fs2){
					$fs = $fs1;
					$pi = (0.5*$father_mutation*0.5*0.1**($fs-1))/$af_table{$locus}->{$pog1};
				}elsif($fs1 > $fs2){
					$fs = $fs2;
					$pi = (0.5*$father_mutation*0.5*0.1**($fs-1))/$af_table{$locus}->{$pog1};
				}else{
